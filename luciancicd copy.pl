...
%trace,
 ...
 %trace,
 %findall(_,(member([K2,Mod_time52],Mod_times),
trace,write_vfs_s(FZ,write,T11),
...
findall(Result,(member([Go_path1,File,Command0],Tests0),vfs2(Command0,Command),
%trace,
...
%%%DELcatch(bash_command1(S3,_), _, (foldr(string_concat,["Warning."%%"Error: Can't clone ",User3,"/",Repository3," repository on GitHub."
	...
	%%%DELfail%abort
 	...
 	foldr(string_concat,X27,X28),
 	split_on_substring117a(X28,".\n",X29),
 	split_into_assertable_predicates(X29,X30),
 	findall(_,(member(X31,X30),retractall(X30),
 	assertz(X30)),_),
	...
	findall(_,(member(X31,X30),retractall(X30)),_)
